...
/**
 * This DataTypeConverter is used to Convert a Float Value into the BigDecimal Datatype by normally using the convert
 * method.
 * 
...
public class FloatToBigDecimalDatatypeConverter implements DatatypeConverter<Float, BigDecimal> {
  ...
  /**
   * Method converts the passed Float object to a BigDecimal.
   * 
   * @param pInput is the value which will be converted into the BigDecimal datatype.
   * @return {@link BigDecimal} BigDecimal object to which the passed Float was converted. The method returns null in
   * the case the pInput is null.
   */
  public final BigDecimal convert( Float pInput ) {
    // No checks of parameter required.
    BigDecimal lOutput;
    if (pInput != null) {
      lOutput = BigDecimal.valueOf(pInput.doubleValue());
    }
    else {
      lOutput = null;
    }
